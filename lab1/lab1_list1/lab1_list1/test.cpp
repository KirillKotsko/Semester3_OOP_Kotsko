...
void TestQueue() {
	...
	}
	{
		...
		ASSERT_EQUAL(test.size(), 2);
		ASSERT_EQUAL(test.empty(), 0);
		ASSERT_EQUAL(test.pop_front(), 6);
		ASSERT_EQUAL(test.pop_front(), 10);
		ASSERT_EQUAL(test.size(), 0);
		ASSERT_EQUAL(test.empty(), 1);
	}
}
